...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['save_mpesa'])) {
        $db->prepare("UPDATE settings SET mpesa_consumer_key=?, mpesa_consumer_secret=?, mpesa_shortcode=?, mpesa_passkey=?, mpesa_env=?, callback_url=? WHERE id=1")
           ->execute([$_POST['consumer_key'], $_POST['consumer_secret'], $_POST['shortcode'], $_POST['passkey'], $_POST['env'], $_POST['callback_url']]);
        $success = "M-Pesa settings saved!";
    }

    if (isset($_POST['save_general'])) {
        $db->prepare("UPDATE settings SET business_name=?, business_phone=?, business_email=?, currency=?, timezone=? WHERE id=1")
           ->execute([$_POST['business_name'], $_POST['business_phone'], $_POST['business_email'], $_POST['currency'], $_POST['timezone']]);
